...
if (isProduction && !allowedOrigin) throw new Error("FRONTEND_URL must be configured in production.");
const corsOptions = isProduction ? { origin: allowedOrigin } : { origin: allowedOrigin || true };
...
app.use((err, req, res, next) => {
  ...
  res.status(err.status || 500).json({ message: isProduction ? "An unexpected server error occurred." : err.message || "Internal server error." });
});
