...
const addClass = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO classes (organization_id, name, code, description, academic_year) VALUES ($1,$2,$3,$4,$5)
...
const getClassDetails = async (req, res) => {
  try {
    const details = await loadClassDetails(req.params.id, organizationId(req));
    if (!details) return res.status(404).json({ message: "Class not found." });
    res.json(details);
  } catch (error) { console.error(error); res.status(500).json({ message: "Failed to load class details." }); }
};
...
const replaceClassTeachers = async (req, res) => {
  ...
  try {
    ...
    for (const id of uniqueIds) await client.query("INSERT INTO class_teachers (class_id, teacher_id) VALUES ($1,$2)", [req.params.id, id]);
...
const replaceClassSubjects = async (req, res) => {
  ...
  try {
    ...
    for (const id of uniqueIds) await client.query("INSERT INTO class_subjects (class_id, subject_id) VALUES ($1,$2)", [req.params.id, id]);
