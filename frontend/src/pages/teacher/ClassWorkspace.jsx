...
function ClassWorkspace() {
  ...
  return (
    <DashboardLayout role="teacher">
      ...
      <div className="tab-nav" style={{ display: "flex", gap: 8, margin: "20px 0", flexWrap: "wrap" }}>
        {tabs.map((item) => <button key={item} type="button" className={tab === item ? "login-btn" : "action-btn"} onClick={() => setTab(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}
      </div>
