    ...
    <DashboardLayout role="teacher">
      <div className="dashboard-page class-workspace-page">
        <div className="dashboard-header">
          <div>
            <Link to="/teacher/dashboard">← My classes</Link>
        ...
        <div className="tab-nav">
          {tabs.map((item) => (
            <button key={item} type="button" className={tab === item ? "login-btn" : "action-btn"} onClick={() => setTab(item)}>
        ...
        {loading ? <div className="section-card"><p>Loading workspace...</p></div> : (
          <div className="dashboard-sections">
            {tab === "overview" && <>
              <div className="section-card"><h2>Assignments</h2><p>{data.assignments.length} published assignment{data.assignments.length === 1 ? "" : "s"}</p>{errors.assignments && <div className="inline-warning">{errors.assignments}</div>}</div>
              <div className="section-card"><h2>Announcements</h2><p>{data.announcements.length} class update{data.announcements.length === 1 ? "" : "s"}</p>{errors.announcements && <div className="inline-warning">{errors.announcements}</div>}</div>
              <div className="section-card"><h2>Resources</h2><p>{data.resources.length} learning resource{data.resources.length === 1 ? "" : "s"}</p>{errors.resources && <div className="inline-warning">{errors.resources}</div>}</div>
              <div className="section-card"><h2>Class Chat</h2><p>Talk with your students and other members connected to this class.</p><button type="button" className="action-btn" onClick={() => setTab("chat")}>Open chat</button></div>
            </>}

            {tab === "assignments" && <div className="section-card"><h2>Assignments</h2>{errors.assignments ? <div className="error-message">{errors.assignments}</div> : data.assignments.length ? data.assignments.map((assignment) => <article key={assignment.id}><h3>{assignment.title}</h3><p>{assignment.instructions || "No instructions provided."}</p><small>{assignment.subject_name || "General"} · {assignment.max_marks} marks{assignment.due_at ? ` · Due ${new Date(assignment.due_at).toLocaleString()}` : ""}</small><hr /></article>) : <p>No published assignments yet.</p>}</div>}
