...
function ClassWorkspace() {
  ...
  return <DashboardLayout role="student">
    ...
    <div style={{ display: "flex", gap: 8, margin: "20px 0", flexWrap: "wrap" }}>
      {tabs.map((item) => <button type="button" key={item} className={tab === item ? "login-btn" : "action-btn"} onClick={() => setTab(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}
    </div>
    {loading ? <div className="section-card"><p>Loading your class...</p></div> : <div className="dashboard-sections">
      {tab === "overview" && <><div className="section-card"><h2>Assignments</h2><p>{data.assignments.length} published</p></div><div className="section-card"><h2>Announcements</h2><p>{data.announcements.length} posted</p></div><div className="section-card"><h2>Resources</h2><p>{data.resources.length} available</p></div><div className="section-card"><h2>Class Chat</h2><p>Ask questions, share updates, and communicate with your class.</p><button type="button" className="action-btn" onClick={() => setTab("chat")}>Open chat</button></div></>}
